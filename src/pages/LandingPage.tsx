...
export default function LandingPage() {
  ...
  const [avgGrades, setAvgGrades] = useState<any>({});
  ...
  useEffect(() => {
    ...
    const fetchProfData = async () => {
      try {
        ...
        const grades: any = {};
        const coursePromises = (prof.courses || []).map(async (course: string) => {
      ...
      <div className="grid grid-rows-[10fr_13fr] min-h-screen">
        <div className="w-full relative overflow-hidden">
          ...
          <div className="w-[37.8%] p-24">
            <h1 className="text-white text-[54px] font-bold relative">Find My Professor</h1>
            <br />
            <h2 className="text-white text-[22.5px] font-regular relative">Want to know more about the professors here at UMD? This is the perfect place to learn a bit about the courses they teach and their grade distributions.</h2>
          </div>
        </div>

        <div className="bg-white w-full relative">
          <h1 className="absolute left-1/2 top-[12%] -translate-x-1/2 text-black font-bold text-[36px]">Enter a Professor to start</h1>

          <form role="search" onSubmit={handleSubmit} className="absolute left-1/2 top-[22%] -translate-x-1/2 w-[70%] max-w-3xl">
            <div className="flex">
              <input name="name" type="text" placeholder="Enter professor name..." className="flex-1 px-6 bg-white py-3 border rounded-l-xl text-black" onChange={handleChange} />
